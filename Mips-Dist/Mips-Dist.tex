...
Moreover, it requires less sub-queries federation to achieve the same recall as existing approaches, thus improving both execution time and network traffic.
...
We call triples that can be found in several knowledge bases across the Web of Data \emph{duplicates}. It is important to make a distinguish between fragmented and duplicated data. In fragmented data, different set of information about the same resource is stored in different datasets depending upon the requirements. For example the various type of information about a single drug can be collected from different datasets in the Linked open Data Cloud. While duplicate data mean, the same set of information about a resource is stored in multiple data sets available via different SPARQL endpoints.
