...
%}
\begin{abstract}
Over the last years, the Web of Data has developed into a large compendium of interlinked data sets from multiple and independent domains.
Due to the lack of coordination, several of these data sets might contain duplicate data. 
...
The ranking function implemented by our approach allows taking into account the overlap among the available data sources and thus avoid submitting sub-queries that produces  overlapping results.  
By these means, our approach applied to existing federated approaches can achieve similar performance while using fewer sub-queries, thus improving both execution time and network traffic. 
...
Moreover, it requires less sub-queries federation to achieve the same recall with smaller execution time.
