...
This work presents DAW2, a novel duplicate-aware approach to federated querying. 
DAW2 is based on a combination of Min-Wise Independent Permutations and data summary statistics applied to RDF predicates.
...
It allows taking into account the overlap among the available data sources and thus avoid submitting sub-queries that produce overlapping results.  
...
Consequently, DAW2 can reduce both the overall runtime required by federated query engines and the amount of network traffic they generate.
...
We extend both DARQ and SPLENDID with DAW2 and compare our extensions with the original frameworks on four datasets of different sizes by using 79 queries of various shapes. 
...
Moreover, our extensions require less sub-queries and smaller execution times to achieve the same recall.
\keywords{Federated queries, SPARQL, Min-Wise Independent Permutations}
\end{abstract}
%

\section{Introduction}
Over the last years, the Data Web has developed into a large compendium of interlinked data sets from multiple domains. 
The central principles underlying the architecture of these data sets include the decentralised provision of data, the reuse of URIs and vocabularies as well as the linking of knowledge bases~\cite{AUE+11}.
A result of this architectural choice is that certain queries can only be answered by retrieving information from several knowledge bases.
...
Due to the reuse of URIs and the independence of the data sources, certain pieces of information (i.e., RDF triples) can be found in several knowledge bases. 
...
%Similarly, whole datasets can be duplicated. 
For example, all triples from the \emph{DrugBank}\footnote{\url{http://datahub.io/dataset/fu-berlin-drugbank}} and \emph{Neurocommons}\footnote{\url{http://neurocommons.org/page/RDF\_distribution}} 
datasets can also be found in the \emph{DERI health Care and Life Sciences Knowledge Base}\footnote{\url{http://hcls.deri.org:8080/openrdf-sesame/repositories/hclskb}}. 
...
While the importance of federated queries over the Web of Data has been stressed in previous work~\cite{key-41,key-7,key-2}, the impact of duplicates has not received much attention. 
Yet, avoiding the retrieval of duplicate triples is a crucial issue when querying distributed datasets for two main reasons:
First, a duplicate-aware approach for federated querying can lead to sub-queries which would not augment the overall recall of a query engine (QE) being discarded.
This leads to the composition of the final result set of the QE being less resource-consuming.
Moreover, the network traffic engendered by the QE can be reduced.
We thus present DAW2, a duplicate-aware approach for query processing. 
Our approach makes use of a combination of Min-Wise Independent Permutations (MIPs) and data summary statistics to estimate the overlap between the results of different sub-queries and discard sub-queries that would lead to redundant information.

 %
%As the SPARQL endpoints are autonomous, there is always the possibility of high mutual overlap between different datasets residing in different endpoints. 
...
\item We combine data summaries statistics with MIPs to address the diversity in SPARQL queries. %this is not clear
\item We implement our approach by extending the federated query engines DARQ and SPLENDID. 
A comparison of our extensions with the original frameworks shows that we achieve better execution times, require fewer sub-queries and consequently engender less network traffic.
\end{enumerate}
...
Subsequently, we present a thorough evaluation of DAW2.
We finally conclude the paper with a discussion of our findings.
...
the index needs to be constantly updated to ensure complete query results.
...
without using any stored data summaries. The data source statistics can be collected on-the-fly before the query federation. This approach promises the complete records retrieval however; it may increase the query execution time, depending on the extra processing required in collecting, processing on-the-fly statistics.
...
%index assisted contributions
Quilitz and Leser~\cite{key-2} develop the first index-assisted federated
query engine (named DARQ) for remote RDF data sources. DARQ uses service
descriptions for relevant data source selection. They use a query rewriting
...
Their experimental results show that the optimization algorithm
can greatly reduce the query processing time. However, it only works for SPARQL queries with bound predicates.
Langegger et al.~\cite{key-3} propose a solution similar to DARQ using a
...
Harth et al.~\cite{key-5}, propose a Qtree-based index structure which summarizes the
content of data source for query execution over the Web of Data. The index leads to efficient source selection for conjunctive query answering however; because of the complete-query based source ranking, it may skip querying many sources which are capable of answering a sub-set of the query triples. Li and Heflin~\cite{key-15} build a tree structure which supports federated query processing over multiple heterogeneous sources. They make use of an OWL reasoner to answer queries over the selected sources and their corresponding ontologies. 
 Görlitz et al.~\cite{key-41} make use of the VOID descriptions for relevant data source selection along with SPARQL \emph{ASK} queries. A dynamic Dynamic Programming~\cite{key-42} strategy is used to optimize the join order of SPARQL basic graph patterns.

%index free contributions
Schwarte et al.~\cite{key-7} propose FedX, an index-free query federation for the Web
of Data. Source selection is performed using SPARQL \emph{ASK} queries. Their approach gives a reasonably fast data retrieval as
compared to all the previous techniques.
Avalanche~\cite{key-24} gathers endpoints dataset statistics and bandwidth availability on-the-fly before the query federation. It provides live query execution over the Web of Data. 

%hybrid approaches
Ludwig and Tran~\cite{key-18} propose a mixed
query engine that assumes some incomplete knowledge about the sources to select and discover new sources at run time. A Symmetric Hash Join is implemented to incrementally produce answers. This approach
is extended to query both remote and local linked data~\cite{key-19}. Acosta et al.~\cite{key-21} present ANAPSID, an adaptive query engine that adapts query execution schedulers to SPARQL endpoints data
...
is bursty. The operators produce results as quickly as data arrives from the sources. 
...
The set of all possible result sets is denoted $R$ while the set of all possible SPARQL queries is labelled with $Q$. 
...
We compared our approach with DARQ, a well-known, publicly available and widely used federated query execution engine.
...
In future work, we will extend our approach by combining it with time-efficient duplicate detection systems such as LIMES~\cite{key-26}. 
