...
One of the central principles underlying the architecture of these data sets is the reuse of URIs and vocabularies as well as the linking of knowledge bases~\cite{AUE+11}.
...
For example, the \emph{DrugBank}\footnote{\url{http://datahub.io/dataset/fu-berlin-drugbank}} and \emph{Neurocommons}\footnote{\url{http://neurocommons.org/page/RDF\_distribution}} 
datasets are duplicated at \emph{DERI health Care and Life Sciences Knowledge Base}\footnote{\url{http://hcls.deri.org:8080/openrdf-sesame/repositories/hclskb}}. 
...
First, a duplicate-aware approach for federated querying can lead to sub-queries that would lead to the same result set as other sub-queries being discarded.
This leads to the composition of the final result set of the federated engine being less resource-consuming.
Moreover, the network traffic engendered by the approach can be reduced.
In this paper, we address this drawback by presenting DAW2, a duplicate-aware approach for query processing based on Min-Wise Independent Permutations (MIPs). 
Our approach makes use of a combination of MIPs and data summary statistics to estimate the possible overlap between the results of different sub-queries in order to avoid the retrieval of redundant information.
